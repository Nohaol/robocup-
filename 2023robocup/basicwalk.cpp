 ...
 * Stance parameters
 */
double bodyHeight = 0.37;//重心高度       fanxiang
double bodyTilt =  7 *PI / 180;/*倾斜*/     //fanxiang
double footX = 0.04;
double footY = 0.002;
double supportX = 0.02;
double supportY = 0.05;
...
//Gait parameters布态参量
//重要的步行参量，需经常更改，放此以便
double tStep0 = 0.34;//机器人踏一步的周期fanxiang
double tStep = 0.34;//机器人踏一步的周期fanxiang
double tZmp = 0.34;//0.1466;//0.165-tStep.6;0.245?-tStep.3;    此处理论上为sqrt(Zc/g)......但实际上为理论值的二倍会很理想。另外此值对步态的整体效果有很大影响，可在理论值得二倍左右进行微调。
////the less tZmp, the more bias
double stepHeight = 0.4;//抬脚高度fanxiang
double  ph1Single =1.3;//单脚站立的时间
	double  ph2Single =1.1 ;//双脚站立的时间
double ph1Zmp = 0.1;
double ph2Zmp = 0.9;//一般不更改

//Compensation parameters修正参数
double hipRollCompensation = 0* PI / 180; //左右摇摆可改变此参数fanxiang
...
Vector3D velDiff = Vector3D(0, 0, 0);//速度差值
//
//ZMP exponential coefficients:
double aXP = 0, aXN = 0, aYP = 0, aYN = 0;

//Gyro stabilization variables
double LankleShift[2] = { 1 * PI / 180, 1 * PI / 180 };//fanxiang
double RankleShift[2] = { 1 * PI / 180, 1 * PI / 180 };//fanxiang
double kneeShift = 0;//fanxiang
...
double hipShift[2] = { 1.1*PI/180,0*PI/180 };  //0控制前后+前-后；1控制左右 -左+右fanxiang
double hipShift3 = 0*PI/180;    //控制左右转动     +右转且平移 -左转且平移//TODO 不 合理fanxiang
...
char device0[20];
int angle[12] = {
				//2068,2038,2119,2188,1844,2028,       //和angleinit一起调
		   //1961,2088,2038,1888,2236,2068
            1958,2018,1938,1838,1368,2048,
		   2048,2018,2108,2378,2178,2048

		};
int ID_head[2] = { 20, 21 };
int angleOfHead[2] = {2028,512};
int angleOfHeadInit[2] = {2028,512};
int ID[] = { 8, 9, 10, 11, 12, 13, 2, 3, 4, 5, 6, 7 };  //No.3 and 9 motor was exchanged
int ID_hands[6] = { 14, 15, 16, 17, 18, 19 };
int angleinit[] = {                                  //chu shi jiao du 同angle
...
*/
		   /*2068,2028,2300,2148,1980,2048,
		   1957,2078,1850,1948,2120,2048*/
		   /*2068,2028,2048,2218,1909,2028,
		   1961,2078,2098,1858,2200,2068*/
		  /* 2068,2028,2119,2188,1844,2028,         //old
		   1961,2108,2038,1888,2236,2068
		    2048,1988,2068,1918,2188,2048,
		    2048,2058,2048,2208,2058,2048*/
1958,2018,1938,1838,1368,2048,
		   2048,2018,2108,2378,2178,2048
...
void serial_legs()
{
		...
		int speed[12] ;//= { 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50 };//2021.4.29
		for(int i=0;i<12;i++)
            speed[i]=400;
...
//			}
			...
			data[10 + 5 * i] = (unsigned char) (speed[i] & 0x00ff);    //speed是啥
			data[11 + 5 * i] = (unsigned char) ((speed[i] >> 8) & 0x00ff);
//			if(i==7){
//				speed[i]=0x00ff;
//			}
//cout<<ID[i]<<"is"<<angle[i]<<endl;
		}
		unsigned char crc = 0;
		for (i = 2; i < 67; i++)
		{
...
void serial_head()
{
	...
	for (i = 0; i < 2; i++)
	{
		...
		//data[10 + 5 * i] = (byte) (0x01);
		//data[11 + 5 * i] = (byte) (0xff);
...
void motion_legs()
{
	...
	if(ph<ph1Single)
		...
		angle[0] = (int) (angleinit[0] - qLegs[0] * 180 / PI * 4095 / 360);//8
		angle[1] = (int) (angleinit[1] + qLegs[1] * 180 / PI * 4095 / 360);//9 dd
		angle[2] = (int) (angleinit[2] + qLegs[2] * 180 / PI * 4095 / 360);//10
		angle[3] = (int) (angleinit[3] + qLegs[3] * 180 / PI * 4095 / 360);//11
		angle[4] = (int) (angleinit[4] + qLegs[4] * 180 / PI * 4095 / 360);//12
		angle[5] = (int) (angleinit[5] - qLegs[5] * 180 / PI * 4095 / 360);//13
		//////////////////////////////////////////////////////////////上为zuo脚，，下为you脚
		angle[6] = (int) (angleinit[6] - qLegs[6] * 180 / PI * 4095 / 360);//2
		angle[7] = (int) (angleinit[7] + qLegs[7] * 180 / PI * 4095 / 360);//3 dd
		angle[8] = (int) (angleinit[8] - qLegs[8] * 180 / PI * 4095 / 360);//4
		angle[9] = (int) (angleinit[9] - qLegs[9] * 180 / PI * 4095 / 360);//5
		angle[10] = (int) (angleinit[10] -qLegs[10] * 180 / PI * 4095 / 360);//6
		angle[11] = (int) (angleinit[11] - qLegs[11] * 180 / PI * 4095 / 360);//7
 serial_legs();
}
...
Vector3D step_torso(Vector3D uLeft, Vector3D uRight, double shiftFactor)//计算torso//shiftfactor keyi gaibain
{
	Vector3D global;
...
{
	 head_angle[0] = min(max(-80.0, lower), 80.0);
	 head_angle[1] = min(max(0.0, upper), 100.0);

	robot_info.q0Real = head_angle[0];

	angleOfHead[0] =(int)((head_angle[0] + 80.0) *  651 / 160.0 + 1820 + 0.5);//(int)((head_angle[0] + 80.0) *  2.844 + 284+ 0.5);
	angleOfHead[1] =(int)((head_angle[1] - 0) * 130 / 100.0 + 456 + 0.5);//(int)((head_angle[1] - 0) * 150 / 100.0 + 366 + 0.5);
    //up_down 161-472
	//L_R 303-732
//	serial_head();
}
