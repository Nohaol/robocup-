...
void dataConvert(struct data duoji, byte b_data[103],int k)
{
	...
	for (int i = 0; i < 21; i++)
	{
			...
			b_data[i * 5 + 10] = (byte) (0x001f); 			//Moving Speed(L)
			b_data[i * 5 + 11] = (byte) (0x0000);
			//b_data[i * 5 + 10] = (byte) (duoji.speed[i] & 0x00ff); 			//Moving Speed(L)
			//b_data[i * 5 + 11] = (byte) ((duoji.speed[i] >> 8) & 0x00ff); 	//Moving Speed(H)
            if(k==4)
            {
            	b_data[i * 5 + 10] = (byte)(0xff); 			//Moving Speed(L)
            	b_data[i * 5 + 11] = (byte)(0x01);          //改舵机速度
            }
