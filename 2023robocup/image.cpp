...
void 			Image_round_goal(void)
{
	...
	Image_Run();
	if(robot_info.ball_dis4robot>20)  	robot_info.ball_find = get_ball_pos(mat);
	else	robot_info.ball_find = get_near_ball_pos(mat) ;
	//robot_info.ball_find = get_near_ball_pos(mat) ;
	if(!robot_info.ball_find){
		if(robot_info.ball_info.x < 160){
...
//	cvNamedWindow("show_Image",1);
	...
	#endif
//cvShowImage("show_Image", Image ); // 在对应的图像窗口打开image
//cvWaitKey(0); // 等待键盘操作  如果是正数x，等待x毫秒
//cvDestroyAllWindows(); // 关闭所有图像窗口
//cvReleaseImage(&Image); // 释放内存
	...
	//figure_Image=cvCreateImage(cvGetSize(pImage),pImage->depth,pImage->nChannels);
...
void Image_Run()
{
	...
	//
		...
		for( int i = 0; i < lines->total; i=i+2 )  //lines存储的是直线
		{
	...
	cvErode(ball_Image_gray,ball_Image_gray);
...
//	Read_near_colorTable
	//cout<<"r:"<<ball_R<<endl;
	if((time0==0||get_time()-time0>=10)&&ball_R>36&&ball_R_Last<=50)
	{
		time0=get_time();
...
// 本程序是靠颜色区分足球与草坪的，所以只能检测出绿草坪上的黑白足球
...
//void Color_Table_figure_ball(IplImage *src )
//{
//	CvSize  size=cvGetSize(src);
//	IplImage *temp_image=cvCreateImage(cvGetSize(pImage),pImage->depth,3);
////	long i;
...
//    cvSmooth(field_Image,field_Image,CV_MEDIAN,3);
//    cvSmooth(ball_Image,ball_Image,CV_MEDIAN,3);
//    cvSmooth(figure_Image,figure_Image,CV_MEDIAN,3);
//   // cvMorphologyEx(field_Image,field_Image,temp_image,NULL,CV_MOP_ERODE,1);
//   //
//}
...
//对mat,ball_Image_gray,ball_Image_gray1做球的识别
		...
		//    	 cout<<robot_info.ball_dis4robot<<endl;
		 ...
		 else if(get_ball_pos_3(ball_Image_gray)==true)
		 	 {
		    	t_begin = get_time();
		    	 ...
		    	 robot_info.calc_ball_info();
		    	 //cout<<"dis1:"<<robot_info.ball_dis4robot<<endl;
		    //cout<<"firstfirstfirst"<<endl;
		 	 ...
		 	 else if(get_ball_pos_3(ball_Image_gray1)==true)
		 	 {
		    	t_begin = get_time();
		    	 ...
		    	 robot_info.calc_ball_info();
		    	 //cout<<"dis2:"<<robot_info.ball_dis4robot<<endl;
...
//		 	 else if(get_ball_pos_3(figure_Image)==true)
//
//		 				    	t_begin = get_time();
//		 				    	 cvCircle(show_Image,cvPoint(robot_info.ball_info.x,robot_info.ball_info.y),robot_info.ball_info.r,cvScalar(255,255,0),2,8,0);
//		 				  //  	 robot_info.calc_ball_info();
...
bool get_ball_pos_1(CvMat *src)
{
	...
	const double area_ratio_limit = 0.50;	//可调
		CvPoint2D32f center;
		float real_radius=0;
		double cont_area;
		double circle_area;
	...
	//		cout<<"cont_total="<<cont->total<<endl;
			//cout<<"r1:"<<real_radius<<endl;
			if(real_radius < 20 || real_radius > 50){
//				cout<<"r2:"<<real_radius<<endl;
//				cout<<real_radius<<endl;
				continue;
			}
			//cout<<"real_radius: "<<real_radius<<endl;
...
{
	const double area_ratio_limit = 0.5;	//可调
		CvPoint2D32f center;
		float real_radius=0;
		double cont_area;
		double circle_area;
...
//param2这个参数表示在同一条直线上进行碎线段连接的最大间隔值(gap), 即当同一条直线上的两条碎线段之间的间隔小于param2时，将其合二为一
{
			...
			float degree=70;
			//cvCanny( goal_Image_gray, goal_canny_Image, 100, 200, 3 );//边缘检测
			cvCanny( goal_Image_gray, goal_canny_Image, 90, 200, 3 );//边缘检测   120 200
			//cvShowImage("cv",goal_canny_Image);
			//cout<<"cvanny ok";
			//cvSobel(goal_Image_gray, goal_canny_Image,1,0);
			//goal_canny_Image=goal_Image_gray;
			cvClearSeq(lines);
		...
		//	循环直线序列，将所有直线标记出来
			...
			{
						line = ( CvPoint* )cvGetSeqElem( lines, i );  //lines序列里面存储的是像素点坐标
						cvLine( goal_color_dst, line[0], line[1], CV_RGB( 255, 0, 0 ) );  //	color_dst是三通道图像用来存直线图像将找到的直线标记为白色
			}
...
//				cout<<"one goal is found"<<endl;
			...
			cvLine(goal_color_dst,cvPoint(goal_point[0][0],goal_point[0][1]),cvPoint(goal_point[1][0],goal_point[1][1]),CV_RGB(255,255,255));
...
float aa1[3],ww[3],Anglee[3],hh[3];
int counttime=0,catchtime=0;
...
void ParseData(char chr)
{
    ...
    memcpy(&sData[0],&chrBuf[2],8);
		...
		{
				case 0x51:
					for (i=0;i<3;i++) aa1[i] = (float)sData[i]/32768.0*16.0;
					time(&now);

                  // {printf("\r\nT:%s a:%6.3f %6.3f %6.3f ",asctime(localtime(&now)),aa1[0],aa1[1],aa1[2]);cout<<endl;}
					break;
				case 0x52:
					for (i=0;i<3;i++) ww[i] = (float)sData[i]/32768.0*2000.0;

                   //{printf("w:%7.3f %7.3f %7.3f ",ww[0],ww[1],ww[2]);cout <<endl;}
					break;
				case 0x53:
					for (i=0;i<3;i++) Anglee[i] = (float)sData[i]/32768.0*180.0;

				//{printf("A:%7.3f %7.3f %7.3f ",Anglee[0],Anglee[1],Anglee[2])cout<<endl;}

					break;
				case 0x54:
					for (i=0;i<3;i++) hh[i] = (float)sData[i];

               //{ printf("h:%4.0f %4.0f %4.0f ",hh[0],hh[1],hh[2]);cout<<endl;}
