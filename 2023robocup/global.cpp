...
double DisMin1 = 20;   //追球时最小距离（修改）×××××××××××××××××××××××××××××
double DisMin = 9;  //调整球时最小距离（修改）×××××××××××××××××××××××××××××

 //摄像头角度
 double head_angle[2] = {0, 0};

 double T_lower = 16;//16		//锯齿波周期
 double T_upper = 4;//4	//正弦波周期
...
//int green_points_up_3= 105;
CvCapture* capture;
IplImage *show_Image,*Image,* pImage,*ball_Image,*ball_Image_gray,*figure_Image,*field_Image,*field_Image_gray,*goal_canny_Image,*goal_color_dst,*goal_Image_gray,*goal_Image,*ball_Image_gray1;						//HSVImagegray和white_cvtImage分别是匹配完绿色和白色的灰度图
